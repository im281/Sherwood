...
The object oriented framework contained within this directory forms the basis of a command line demo tool that accompanies Sherwood (and is required to complete the exercises in Part I of the book). The demo provides a useful illustration of how the framework can be applied simply to a variety of different decision forest inference tasks, and may be a useful place to get started (see \csharp\demo\ReadMe.txt).
...
To use Sherwood's object oriented decision forest framework within your own .NET project, all that is necessary is to reference the associated assembly (Sherwood.dll). A pre-built binary version (for .NET 4.0, AnyCPU) is available in /csharp/bin/net4.0. Alternatively build instructions (for Mono and C#) are the same as those for the command line demo that accompanies Sherwood (/csharp/demo/ReadMe.txt).

To use the object oriented framework in your problem domain, the following steps will be required:
...
2. Use the ForestTrainer::TrainForest() method to create a new Forest.  Alternatively, if you have compiled the ParallelForestTrainer class (which requires .NET 4.0) and you wish to parallelize the node training over candidate features, you could call ParallelForestTrainer.TrainForest()).
...
For an example of how the framework has been adapted to a toy classification problem, please see the classes defined in Classification.cs in the /csharp/demo/source subdirectory.

For more information about how to use the framework, please see the book.
